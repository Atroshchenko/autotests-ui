...
with sync_playwright() as playwright:
    ...
    context.storage_state(path='test-browser-state.json') # Создаём файл с сохраненным состоянием (данные local storage)

with sync_playwright() as playwright:
    browser = playwright.chromium.launch(headless=False)
    context = browser.new_context(storage_state='test-browser-state.json') # Указываем файл с сохраненным состоянием
    ...
    result_block = page.get_by_test_id('courses-list-empty-view-description-text')
